...
void ListLinkedSingle::duplicate() {
    ...
    //Deberia de funcionar para listas unitarias y vacias

    Node* curr = head; //Nodo actual a duplicar, empezamos por la cabeza
    while (curr != nullptr) {
        Node *dup = new Node;
        dup->value = curr->value;
        dup->next = curr->next;
        curr->next = dup;
        curr = dup->next;    //Movemos curr
    }
}
