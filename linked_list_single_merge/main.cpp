...
void ListLinkedSingle::merge(ListLinkedSingle& other) {
    ...
    //Decidimos por que lista comenzar
    ...
    //Vamos saltando de una lista a otra
    while (cur->next != nullptr) {
        if (cur->next->value <= aux->value) {
            cur = cur->next;//Seguimos en la lista principal moviendonos
        }
        else { //Es menor el de la otra lista
            Node* old_next = cur->next;
            cur->next = aux; //apunta curr a la otra lista
            cur = aux;  //Actualizamos curr
            aux = old_next; //Aux apunta a donde apuntaba curr
            //cur = cur->next; //Actualizamos curr
        }
    }
    cur->next = aux; //Apunto a la otra lista si es que queda algo en ella
