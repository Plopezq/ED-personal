...
public:
    ...
    void invSeg(int comienzo, int longitud);
...
template<typename Elem>
void ListLinkedSingle<Elem>::invSeg(int comienzo, int longitud){ //Invertir un segmento
   //NO se puede usar new
    Node* auxInter = head->next;
    //lo coloco CASI al comienzo del intervalo
    for (int i = 1; i < (comienzo - 1); i++ ) {
        auxInter = auxInter->next;
    }

    //INVERTIMOS ESE INTERVALO
    Node* curr = auxInter;
    Node* prev = nullptr;
    int segLong = 0;
    while (segLong < longitud) { //Tantas veces como longitud tenga el intervalo
        Node* old_next = curr->next;
        curr->next = prev; //Apuntamos al anterior
        prev = curr; //Actualizamos prev que ahora apunta a curr
        curr = old_next;
        segLong += 1;
    }
    //algo
