...
public:
    ...
    void merge(ListLinkedSingle<int>& other);
...
template<typename Elem>
void ListLinkedSingle<Elem>::merge(ListLinkedSingle<int>& other){
   //La lista pasada como argumento pasara a estar vacia
    Node* cur = nullptr;
    Node* aux = nullptr;
    ...
    if (this->head->next->value <= other.head->next->value) {//por los nodos fantasma
        cur = this->head->next;
        aux = other.head->next;
    }
    else {
    ...
    cur->next = aux;//para los que queden
    other.head->next = nullptr;
