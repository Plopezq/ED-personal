...
template <typename T> ListLinkedDouble<T> ListLinkedDouble<T>::split(const int indice) {
    ListLinkedDouble lista;
    if (indice == num_elems) return lista;
    int i = 0;
    Node* cur = this->head;
    while (i <= indice) {
        cur = cur->next;
    ...
    lista.head->next = cur;
    lista.num_elems = num_elems - indice + 1;
    lista.head->prev = last;
