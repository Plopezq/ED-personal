...
BinTree<int> reconstruir(const vector<int>& preorden,
    const vector<int>& inorden, int& p, int& i, int max) {
    // Completar

    if (p == preorden.size() || inorden[i] == max) return {}; // vacío
    //cogemos la raiz
    int root = preorden[p];
    ++p;
    auto arbol_iz = reconstruir(preorden, inorden, p, i, root);
    ++i;
    auto arbol_dr = reconstruir(preorden, inorden, p, i, max);

    return{ arbol_iz,root,arbol_dr };
...
bool tratar_caso() {
    int numNodos;
    cin >> numNodos;
    if (numNodos == 0) return false;

    vector<int> preorden(numNodos);
    vector<int> inorden(numNodos);

    //guardamos el recorrido preorden
    for (int i = 0; i < numNodos; i++) {
        cin >> preorden[i];
    }
    //guardamos el recorrido inorden
    for (int i = 0; i < numNodos; i++) {
        cin >> inorden[i];
    }
    ...
    BinTree<int> tree = reconstruir(preorden, inorden, p, i, std::numeric_limits<int>::min());
